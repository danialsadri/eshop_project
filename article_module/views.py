...
class ArticlesListView(ListView):
    ...
    def get_queryset(self):
        ...
        if category_name is not None:
            query = query.filter(categories__url_title__iexact=category_name)
        return query
...
def add_article_comment(request: HttpRequest):
    if request.user.is_authenticated:
        ...
        ArticleComment.objects.create(article_id=article_id, text=article_comment, user_id=request.user.id, parent_id=parent_id)
        context = {
            'comments': ArticleComment.objects.filter(article_id=article_id, parent=None).order_by('-create_date').prefetch_related('comments'),
...
def article_categories_component(request: HttpRequest):
    article_main_categories = ArticleCategory.objects.prefetch_related('categories').filter(is_active=True, parent_id=None)
