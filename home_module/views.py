...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        most_bought_products = Product.objects.filter(order_details__order__is_paid=True).annotate(order_count=Sum('order_details__count')).order_by('-order_count')[:12]
        ...
        for category in categories:
            item = {'id': category.id, 'title': category.title, 'products': list(category.product_categories.all()[:4])}
